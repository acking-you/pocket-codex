...
// The fixed-size key's alignment varies across 32-bit targets: the backing
// allocation occupies 40 bytes on ARMv7 and 48 on wasm32. Keep the 32-bit
// footprint bounded without requiring every target to use the same padding.
#[cfg(target_pointer_width = "64")]
static_assertions::assert_eq_size!(PagableArcInner<[usize; 4]>, [usize; 8]);
#[cfg(target_pointer_width = "32")]
static_assertions::const_assert!(
    std::mem::size_of::<PagableArcInner<[usize; 4]>>() <= std::mem::size_of::<[usize; 12]>()
);
