...
| External Codex hosting        | all desktop hosts spawn an installed `codex`; the mobile/desktop bridge uses wire envelopes without linking Codex protocol crates or their runtime dependencies. **Built-in engine** is disabled and not implemented. Existing embedded auto-host settings restore using external Codex, which must be installed or selected by path. |
...
| Native image generation | live generation/loading feedback, inline previews and native file saving, retryable remote artifact reads, and restored/monitored/read-only results; uses the external Codex image tool ([verification](docs/native-image-generation.md)) |
| Session file links | live and read-only session links with explicit text/code/Markdown-source and image previews, streamed remote downloads with legacy Host Files support, mobile Save As, verified local reads, and pb-mapper guidance for remote localhost pages ([details](docs/session-file-links.md)) |
| Persistent controller cache | configurable 512 MB App-wide disk cache, immediate cached history, verified window/text deltas across reconnects, foreground running-session prefetch, and bounded eviction; provider-neutral `SessionHistorySource` adapter ([protocol and limits](docs/session-cache-sync.md)) |
| Turn controls | Auto approval review by default, separate manual/read-only/full-access modes, capability-gated Fast, and running-turn supplements alongside the default next-turn queue; Plan is available under Advanced settings |
