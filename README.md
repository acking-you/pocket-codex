...
| Native image generation | live generation/loading feedback, inline previews and native file saving, retryable remote artifact reads, and restored/monitored results; uses the external Codex image tool ([verification](docs/native-image-generation.md)) |
| Session file links | explicit text/code/Markdown-source and image previews, streamed remote downloads, mobile Save As, verified local reads, and pb-mapper guidance for remote localhost pages ([details](docs/session-file-links.md)) |
